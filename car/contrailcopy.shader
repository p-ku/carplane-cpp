...
//varying float airDamp;
varying float world_x_dot;
varying float airDamp;
varying float findex;
...
//MODELVIEW_MATRIX = INV_CAMERA_MATRIX * mat4(CAMERA_MATRIX[0],CAMERA_MATRIX[1],CAMERA_MATRIX[2],WORLD_MATRIX[3]);
	...
	//int airDamp = INSTANCE_ID;

//	airDamp[INSTANCE_ID] = angDamp;
//	findex = float(INSTANCE_ID);
//int index = INSTANCE_ID;
//airDamp[index] = angDamp;
//COLOR[3] = angDamp;
//airDamp = INSTANCE_CUSTOM[2];
}
...
//airDamp[index] = angDamp;
	...
	//COLOR.r = 1.0-sunSet;
	//COLOR.g = 1.0-sunSet*2.0;
	//COLOR.b = 1.0-sunSet*3.0;
	...
	//ALPHA = clamp((length(world_coord)-27.0)/5.0,0.0,1.0);
	//ALPHA = clamp((length(world_coord)/54.0-1.0)/5.0,0.0,1.0);
	//ALPHA = clamp(airDamp[INSTANCE_ID]/1.0,0.0,1.0);
