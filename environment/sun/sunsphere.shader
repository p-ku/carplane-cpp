...
void fragment()
{
	...
	ALBEDO.r = 1.0;
	ALBEDO.g = (256.0 - sunSet / 2.0) / 255.0;
	ALBEDO.b = (256.0 - sunSet) / 255.0;
	...
	float stepper = UV.x > 0.5 ? step(0.1, length(vec2(UV.x, UV.y))) : step(0.1, length(vec2(UV.x, UV.y)));
	ALPHA = 1. - step(0.1, length(vec2(UV.x, UV.y)));
}