...
uniform vec2 half_inv_reso;
uniform vec2 tile_uv_size;
uniform float half_tile_uv_size_x;
uniform vec2 start_half;
...
void fragment()
{
	...
	vec2 first_pixel = floor(UV / tile_uv_size) * tile_uv_size + half_inv_reso + start_half;
	for (float i = first_pixel.x; i < first_pixel.x + half_tile_uv_size_x; i += inv_reso.x)
	{
