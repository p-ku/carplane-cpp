...
vec2 light_step(vec3 pos, vec3 light_dir, int steps, vec2 scale_height, float radius, float b, float d)
{
...
vec4 cloud_color(vec3 start, vec3 dir, vec2 opt_c, float cloud_ray_length, vec2 density, vec3 ray_c, vec3 mie_c)
{
	...
	if (cloud_den > 0.5)
	{
		...
		//	vec3 colo = cloud_shade * (ray_c * (1. - cloud_den) + mie_c * cloud_den);
		vec3 colo = cloud_shade * (ray_c + mie_c * cloud_den);

		// vec3 colo = cloud_shade * mie_c;

		// colo *= cloud_den; // * intensity;
		// scolo *= (1. - cloud_den);
...
)
{
	...
	if (dCloud > 0.) // clouds possible
	{
		...
		if (cloud_ray_length.x < cloud_ray_length.y) // clouds intercept definite (barring planet in the way)
		{
			if (cloud_ray_length.x > 0.) // above clouds
			{
				...
				//	if (allow_mie)
				//		cloud_stage = 2;
				// cloud_ray_length.y = cloud_ray_length.x;
			}
			else
			{
				if (allow_mie)
	...
	vec3 l_attn = vec3(0.);
	final_light = vec3(0.);
	float tangent_dist = plan_rad * tan(acos(plan_rad / atmo_rad));
	...
	if (cloud_stage > 0)
	{
		vec3 pos_c_one = start + dir * cloud_ray_length.x;
		vec3 pos_c_two = start + dir * cloud_ray_length.y;
		b = 2.0 * dot(light_dir, pos_c_one);
		d = b * b;
		vec2 opt_c_one = light_step(pos_c_one, light_dir, 4, scale_height, cloud_rad, b, d);
		b = 2.0 * dot(light_dir, pos_c_two);
		d = b * b;
		vec2 opt_c_two = light_step(pos_c_two, light_dir, 4, scale_height, cloud_rad, b, d);
		// opt_c_two = opt_c_one;
		...
		else
		{
			...
			//}
			//	else
			//		cloud_path.x = max(steps_raw * ray_length.y / full_path, step_size_i);
		}
		//  cloud_path.z = max(steps_raw * (ray_length.y - cloud_ray_length.y) / full_path, step_size_i);
		//   if (cloud_path.x == half_step_i)
		//	cloud_path.x = 0.;
		//   if (cloud_path.y == half_step_i)
		//	cloud_path.y = 0.;
		//   if (cloud_path.z == half_step_i)
		//	cloud_path.z = 0.;
		// cloud_steps = ivec3(round(cloud_path));
		cloud_steps.x = int(round(cloud_steps_raw.x));
		cloud_steps.y = steps_i - cloud_steps.x;
		// cloud_steps.x = int(round(cloud_path.x));
		// cloud_steps.y = int(round(cloud_path.y));
		// cloud_steps.z = int(round(cloud_path.z));
		...
		for (int i = 0; i < steps_i; ++i)
		{
			//		float step_size_i = full_path / float(steps_i);
			//		float half_step_i = 0.5 * step_size_i;
			//		float rounding = half_step_i * 0.01;
			//
			//		for (int i = 0; i < steps_i + 1; ++i)
			//		{
			...
			if (cloud_stage == 1)
			{
				...
				if (i == cloud_steps.x - 1)
				{
					...
					// sky_color += vec4(ray_constant * cloud_ray[1].xyz + mie_constant * cloud_mie[1].xyz, opacity[1]) * (1. - sky_color.a);
					//  next, add cloud color if it's dense enough
					sky_color += (1. - sky_color.a) * phase_mie * cloud_color(start, dir, opt_c_one, cloud_ray_length.x, density, ray_constant, cloud_mie_constant_one);
				}
			...
			else // if (cloud_stage == 2)
			{
				...
				// if (abs(cloud_ray_length.y - ray_pos_i) < cloud_half_step.y)
				if (i == cloud_steps.y + cloud_steps.x - 1)
				{
					opacity[2] = 1. - length(exp(-(1.1 * beta_mie * opt_i[2].y + beta_ray * opt_i[2].x) * density_multiplier));
					// sky_color += vec4(ray_constant * cloud_ray[2].xyz + mie_constant * cloud_mie[2].xyz, opacity[2]) * (1. - sky_color.a);

					if (allow_mie)
					{
						// cloud_stage = 3;
						sky_color += (1. - sky_color.a) * cloud_color(start, dir, opt_c_two, cloud_ray_length.y, density, ray_constant, cloud_mie_constant_two);
					}
					// else
					//	break;
				}
			...
			//	else
			//	{
			//		pos_i = start + dir * (ray_pos_i); // + cloud_half_step.z);
			//		height_i = length(pos_i) - plan_rad;
			//		vec2 density = exp(-height_i / scale_height) * cloud_step_size.z;
			//		opt_l = light_step(pos_i, light_dir, 4, scale_height, atmo_rad);
			//		opt_i[3].xy += density;
			//		opt_i_tot.xy += density;
			//		attn = exp(-(1.1 * beta_mie * (opt_i_tot.y + opt_l.y) + beta_ray * (opt_i_tot.x + opt_l.x)));
			//		cloud_ray[3].xyz += attn * density.x;
			//		cloud_mie[3].xyz += attn * density.y;
			//		if (i == steps_i - 1)
			//		{
			//			opacity[3] = 1. - length(exp(-(1.1 * beta_mie * opt_i[3].y + beta_ray * opt_i[3].x) * density_multiplier));
			//			sky_color += vec4(ray_constant * cloud_ray[3].xyz + mie_constant * cloud_mie[3].xyz, opacity[3]) * (1. - sky_color.a);
			//		}
			//		ray_pos_i += cloud_step_size.z;
			//	}
			// opacity = 1. - length(exp(-(beta_ray * opt_i[3].x + 1.1 * beta_mie * opt_i[3].y) * density_multiplier ));
			// sky_color += vec4(ray_constant * cloud_ray[3].xyz + mie_constant * cloud_mie[3].xyz,opacity) * (1. - sky_color.a);
		}
		// opacity = 1. - length(exp(-(beta_ray * opt_i_tot.x + 1.1 * beta_mie * opt_i_tot.y) * density_multiplier));
		// sky_color = vec4(ray_constant * cloud_ray[0].xyz + mie_constant * cloud_mie[0].xyz, opacity); // * (1. - sky_color.a);
	...
	else
	{
		for (int i = 0; i < steps_i; ++i)
		{
			...
			opt_i_tot.xy += density;
			attn = exp(-(1.1 * beta_mie * (opt_i_tot.y + opt_l.y) + beta_ray * (opt_i_tot.x + opt_l.x)));
			// if (ray_pos_i == max_dist)
			// if (0 == 1)
			// {
			// 	l_attn = exp(-(1.1 * beta_mie * opt_l.y + beta_ray * opt_l.x));
			// 	final_light = ray_constant * l_attn * density.x + mie_constant * l_attn * density.y;
			// }
			cloud_ray[0].xyz += attn * density.x;
			cloud_mie[0].xyz += attn * density.y;

			//	cloud_ray[0][0] += attn.x * density.x;
			//	cloud_mie[0][0] += attn.y * density.y;
			//	cloud_ray[0][1] += attn.z * density.x;
			//	cloud_mie[0][1] += attn.x * density.y;
			//	cloud_ray[0][2] += attn.y * density.x;
			//	cloud_mie[0][2] += attn.z * density.y;
			// if (i != steps_i)
			ray_pos_i += step_size_i;
			// else
			//	ray_pos_i += ray_length_l - 2. * half_step_i;

			//		ray_pos_i += (step_size_i + half_step_i);
			//	else ray_pos_i += step_size_i;
			//		else ray_pos_i += step_size_i;
		}
		//	vec3 totes_r = vec3(cloud_ray[0][0], cloud_ray[0][1], cloud_ray[0][2]);
		//	vec3 totes_m = vec3(cloud_mie[0][0], cloud_mie[0][1], cloud_mie[0][2]);

		opacity[0] = 1. - length(exp(-(beta_ray * opt_i_tot.x + 1.1 * beta_mie * opt_i_tot.y) * density_multiplier));
		sky_color = vec4(ray_constant * cloud_ray[0].xyz + mie_constant * cloud_mie[0].xyz, opacity[0]);

		//	sky_color += vec4(ray_constant * totes_r + mie_constant * totes_m, opacity); // * (1. - sky_color.a);
	}

	// if( length(exp(-((1.1 * beta_mie * opt_i_tot.y) + (beta_ray * opt_i_tot.x))  ))<0.) break;

	//  if( sky_color.a>0.99 ) break;

	// float opacity = 	1. - length(exp(-((1.1 * beta_mie * opt_i_tot.y) + (beta_ray * opt_i_tot.x)) * density_multiplier ));
	// ec3 opacity = 	vec3(
	//	1. - length(exp(-((1.1 * beta_mie * opt_i[0].y) + (beta_ray * opt_i[0].x)) * density_multiplier )),
	//	1. - length(exp(-((1.1 * beta_mie * opt_i[1].y) + (beta_ray * opt_i[1].x)) * density_multiplier )),
	//	1. - length(exp(-((1.1 * beta_mie * opt_i[2].y) + (beta_ray * opt_i[2].x)) * density_multiplier ))
	//;
	// opacity = min(1., opacity + cloud_den.x + cloud_den.y );
	// float opacity_tot = opacity[0]+opacity[1]+opacity[2]+cloud_den.x + cloud_den.y;
	// total_ray = cloud_ray[0]+cloud_ray[1]+cloud_ray[2];
	// total_mie = cloud_mie[0]+cloud_mie[1]+cloud_mie[2];

	// vec4 col_one = vec4(ray_constant * cloud_ray[0]
	//						 + mie_constant * cloud_mie[0],opacity[0]);
	// vec4 col_two = vec4(ray_constant * cloud_ray[1]*(1.-cloud_den.x)*(1.-opacity[0])
	//						 + mie_constant * cloud_mie[1]*(1.-cloud_den.x)*(1.-opacity[0]),opacity[1]);
	// vec4 col_thr = vec4(ray_constant * cloud_ray[2]*(1.-cloud_den.y)*(1.-cloud_den.x)
	//						 + mie_constant * cloud_mie[2]*(1.-cloud_den.y)*(1.-cloud_den.x),opacity[1]);
	// vec4 cloud_one = vec4(ray_constant * cloud_shade_ray_one
	//							 + mie_constant * cloud_shade_mie_one,cloud_den.x);
	// vec4 cloud_two = vec4(ray_constant * cloud_shade_ray_two
	//							 + mie_constant * cloud_shade_mie_two,cloud_den.y);

	// vec4 skycol;
	// skycol = col_one * (1.-opacity[0]) + col_two*(1- opacity[0] -opacity[2]-cl)
	// vec4 newco_one;
	// newco_one.a = opacity;
	// vec4 fg =
	// newco_one.r = fg.r * fg.a / newco_one.a + bg.r * bg.a * (1 - fg.a) / newco_one.a;
	// newco_one.g = fg.g * fg.a / newco_one.a + bg.g * bg.a * (1 - fg.a) / newco_one.a;
	// newco_one.b = fg.b * fg.a / newco_one.a + bg.b * bg.a * (1 - fg.a) / newco_one.a;
	sky_color.xyz *= intensity;
	return sky_color;
	// return sky_color;return vec4((
	// return sky_color; col_one + col_two + col_thr + cloud_one + cloud_two
	//	) * intensity,opacity_tot);
	// return vec4((
